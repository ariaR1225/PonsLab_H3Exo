...
class spring_controller:
    ...
    def spring_control(self):
        ...
        while not rospy.is_shutdown():
            ...
            #if self.theta > -0.4:
            #torque = self.stiffness*(self.theta+self.knee_theta-self.bias) + self.damping*self.theta_dot
            torque = self.stiffness*(self.theta-self.bias)
            #rospy.loginfo(self.theta)
            ############ minimize torque
            # torque = 5*self.net_torque
            self.torque_cmd = self.torque_cmd*0.3 + self.net_torque*0.7
            #torque = self.torque_cmd*5
            ############
            rospy.loginfo(torque)
            pub_right.publish(-self.torque_cmd*5)
            self.root.update()
            rate.sleep()
            t = t + 1
